...
def write2md(dirpath,title,article):
    ...
    if not os.path.exists(dirpath):# 判断目录是否存在，不存在则创建新的目录
        os.makedirs(dirpath)
    # 创建md文件
    with open('./jianshu-CTF-wp/'+title+'.md','w',encoding="utf8") as f:
...
def get_page_url(question,page_id,mes_id):
    ...
    obj_url=''
    try:
        obj_num=tmp['entries'][mes_id]['slug']
...
def main(argv):
    for x in range(1,30):
        i=0
        while (i<10):
            page_url=get_page_url("CTF%20wp",x,i)
